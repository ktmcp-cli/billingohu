...
import { listBankAccount, createBankAccount, deleteBankAccount, getBankAccount, updateBankAccount, getConversionRate, listDocumentBlock, listDocument, createDocument, getDocument } from './api.js';
...
function requireAuth() {
  if (!isConfigured()) {
    ...
    console.log(chalk.cyan('  billingohu config set --api-key YOUR_API_KEY'));
    process.exit(1);
  }
}
...
program
  .name('billingohu')
  .description(chalk.bold('Billingo API v3 CLI') + ' - Production-ready CLI')
  .version('1.0.0');
...
configCmd
  ...
  .action(() => {
    ...
    console.log(chalk.bold('\nBillingo API v3 CLI Configuration\n'));
...
program
  ...
  .action(async (options) => {
    ...
    try {
      const data = await withSpinner('Making API call...', () => listBankAccount());
